...
# 검색 횟수 카운터 - 파일에 저장해 재시작에도 어느 정도 유지되도록 함
# (단, 무료 호스팅에서 재배포/장시간 슬립 후에는 초기화될 수 있음)
_COUNTER_FILE = Path(__file__).parent / "search_counter.json"


def _load_count():
    try:
        return json.loads(_COUNTER_FILE.read_text()).get("total", 0)
    except Exception:
        return 0
...
def _increment_count():
    count = _load_count() + 1
    try:
        _COUNTER_FILE.write_text(json.dumps({"total": count}))
    except Exception:
        pass
    return count
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .dept-bar { display: flex; align-items: baseline; justify-content: space-between;
              margin: 18px 4px 26px; padding-bottom: 14px; border-bottom: 1px solid var(--line); }
  .dept-name { font-weight: 800; font-size: 14.5px; color: var(--green-dark); letter-spacing: -0.01em; }
  .dept-meta { font-size: 12.5px; color: var(--muted); font-variant-numeric: tabular-nums; white-space: nowrap; }
  ...
  .search-bar input { flex: 1; border: none; outline: none; font: inherit; font-size: 16px; font-weight: 700; background: transparent; color: var(--ink); }
  .search-bar input::placeholder { color: #9AA1A9; font-weight: 400; }
...
def index():
    ...
    total_count = _increment_count() if q else _load_count()
    today = datetime.now().strftime("%Y.%m.%d")

    return render_template_string(
        PAGE, q=q, items=items, error=error, total_cnt=total_cnt, cached=cached,
        today=today, total_count=total_count,
    )
